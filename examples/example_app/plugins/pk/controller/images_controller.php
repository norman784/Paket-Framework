<?php
$PK['PK_Acl']['version'] = '0.0.1';
$PK['PK_Acl']['date'] = '2009-01-05'; 
/**
...
class Images_controller {
	...
	public function indexAction() {
		...
		if (defined('PK_PUBLIC_DIR')) {
			$tmp_file = PK_PUBLIC_DIR.'/images/'.$folder.'/'.$height.'/'.$width;
		}
		
		if (false !== $width && false !== strpos($width, '.'))
			$width = substr($width, 0, strpos($width, '.'));
			
		if (false !== $height && false !== strpos($height, '.'))
		...
		$id = str_replace('-', '/', $folder);
		
//		echo "\n".PK_UPLOAD_DIR.$id."\n<hr>\n";
		
		$file = fexists(PK_UPLOAD_DIR.$id, array('.jpg', '.gif', '.png'));
		if (empty($tmp_file)) {
			$tmp_file = PK_PUBLIC_DIR.'images/'.$id.'_'.$width.'_'.$height.substr($file, -4);
		}
		
		$file_name = $id.'_'.$width.'_'.$height.substr($file, -4);
...
//		exit();
		...
		if (file_exists($tmp_file)) {
			...
			header("Content-Type: " . mime_type($tmp_file));
...
//			$this->Thumbnail($tmp_file);
//			$this->show();
//			$this->destruct();
		} else {
//			echo $tmp_file.'<br>'.$file;
//			exit;
			$this->Thumbnail($file);
			$this->resize($_GET['url']['width'], $_GET['url']['height']);
			$this->save($tmp_file);
			$this->show();
			$this->destruct();
		}
    ...
    function Thumbnail($fileName) {
        ...
        if(!file_exists($this->fileName)) {
            $this->errmsg = 'Archivo no encontrado';
            $this->error = true;
        }
        //check to see if file is readable
        elseif(!is_readable($this->fileName)) {
            $this->errmsg = 'El archivo no es legible';
            $this->error = true;
        }
        ...
        if($this->error == false) {
            ...
            //unknown file format
                ...
                $this->errmsg = 'Formato desconocido';
                $this->error = true;
            }
        }
    ...
    function rotateImage($direction = 'CW') {
		...
		elseif (is_numeric($direction)) {
			...
			$this->workingImage = imagerotate($this->workingImage,$direction,0);
		}
    	else {
    		$this->workingImage = imagerotate($this->workingImage,90,0);
    	}
